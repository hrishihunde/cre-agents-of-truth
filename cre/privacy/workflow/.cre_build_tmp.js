...
var x402RequestSchema = exports_external.object({
  resource_url: exports_external.string().url(),
  method: exports_external.enum(["GET", "POST", "PUT", "DELETE"]).default("GET"),
  parameters: exports_external.record(exports_external.string(), exports_external.any()).optional().default({}),
  payment_proof: exports_external.string().min(1),
  secret_id: exports_external.string().optional()
});
var onHttpTrigger = (runtime2, httpPayload) => {
  let requestData;
  try {
  ...
  const confidentialClient = new ClientCapability;
  const verifyRes = confidentialClient.sendRequest(runtime2, {
    vaultDonSecrets: [],
    request: {
      url: runtime2.config.facilitator_verify_url,
      method: "POST",
